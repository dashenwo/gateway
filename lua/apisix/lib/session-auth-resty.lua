...
function session.new(opts)
    ...
    local ide, iden = prequire("apisix.lib.session-auth.identifiers.", opts.identifier or defaults.identifier, "random")
    local ser, sern = prequire("apisix.lib.session-auth.serializers.", opts.serializer or defaults.serializer, "json")
    local enc, encn = prequire("apisix.lib.session-auth.encoders.",    opts.encoder    or defaults.encoder,    "base64")
    local cip, cipn = prequire("apisix.lib.session-auth.ciphers.",     opts.cipher     or defaults.cipher,     "aes")
    local sto, ston = prequire("apisix.lib.session-auth.storage.",     opts.storage    or defaults.storage,    "cookie")
    local str, strn = prequire("apisix.lib.session-auth.strategies.",  opts.strategy   or defaults.strategy,   "default")
    local hma, hman = prequire("apisix.lib.session-auth.hmac.",        opts.hmac       or defaults.hmac,       "sha1")
    local self = {
        now        = time(),
        name       = name,
        secret     = sec,
